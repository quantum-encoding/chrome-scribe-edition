// Claude Scraper Final Working - Based on the v3 that downloaded 10/10 artifacts
(async function() {
    'use strict';
    
    console.log('🚀 Claude Scraper Final Working\n');
    ...
    async function processArtifacts() {
        ...
        for (let i = 0; i < artifactButtons.length; i++) {
            ...
            // Find dropdown button - look for button with SVG that has the specific arrow path
            ...
            if (!dropdownButton) {
                // Alternative: Look for button with radix ID next to Copy button
                for (const btn of buttons) {
                    if (btn.id && btn.id.startsWith('radix-')) {
            ...
            // THE KEY PART: Search ALL links in the document for "Download as"
