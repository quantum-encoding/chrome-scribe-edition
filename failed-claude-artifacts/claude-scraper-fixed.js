// Claude Scraper Fixed - Ensures proper dropdown click
(async function() {
    'use strict';
    
    console.log('🚀 Claude Scraper Fixed\n');
    ...
    for (let i = 0; i < artifactButtons.length; i++) {
        ...
        // Click the SVG inside the button to ensure proper click registration
        const dropdownSvg = dropdownButton.querySelector('svg');
        if (dropdownSvg) {
            console.log('  Clicking dropdown SVG...');
            dropdownSvg.click();
        } else {
        ...
        // Wait for dropdown menu with increased time
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Look for the dropdown menu - try multiple selectors
        let downloadLink = null;
        
        // Method 1: Look for z-dropdown class
        ...
        for (const menu of dropdownMenus) {
            ...
            for (const link of links) {
                if (link.textContent && link.textContent.includes('Download as')) {
                    downloadLink = link;
                    console.log('  Found in z-dropdown menu');
                    break;
                }
            }
        ...
        // Method 2: Check all body > div elements for new menus
        if (!downloadLink) {
            const bodyDivs = document.querySelectorAll('body > div');
            // Check last 10 divs
            for (let j = Math.max(0, bodyDivs.length - 10); j < bodyDivs.length; j++) {
                const div = bodyDivs[j];
                // Skip if it's one of the main app divs
                if (div.id && div.id.includes('root')) continue;
        ...
        // Method 3: Direct search for download links
        if (!downloadLink) {
            const allLinks = document.querySelectorAll('a[href^="blob:"][download]');
            for (const link of allLinks) {
                // Check if link is visible
                const rect = link.getBoundingClientRect();
                if (rect.width > 0 && rect.height > 0) {
                    downloadLink = link;
                    console.log('  Found visible download link');
                    break;
                }
            }
        }
        ...
        if (downloadLink) {
            console.log(`  ✅ Clicking download: "${downloadLink.textContent.trim()}"`);
            downloadLink.click();
            downloadCount++;
            ...
            const visibleLinks = Array.from(document.querySelectorAll('a')).filter(a => {
                ...
                return rect.width > 0 && rect.height > 0 && a.textContent.includes('Download');
            });
            console.log(`  Debug: Found ${visibleLinks.length} visible links with "Download"`);
        }
