    ...
    async function processArtifacts() {
        ...
        for (let i = 0; i < artifactButtons.length; i++) {
            ...
            if (!downloadLink) {
                ...
                console.log(`  Debug: Found ${allLinks.length} total links in document`);
                // Show a few links for debugging
                const sampleLinks = Array.from(allLinks).slice(0, 5);
                sampleLinks.forEach((link, i) => {
                    console.log(`    Link ${i}: "${link.textContent?.trim().substring(0, 50)}..."`);
                });
            }
            ...
            if (downloadLink) {
                ...
                menus.forEach((menu, idx) => {
                    const id = menu.id || 'no-id';
                    const selector = menu.id ? `#${menu.id}` : `body > div:nth-child(${Array.from(menu.parentElement.children).indexOf(menu) + 1})`;
                    console.log(`    Menu ${idx}: ${selector}, links: ${menu.querySelectorAll('a').length}`);
                });
            }
